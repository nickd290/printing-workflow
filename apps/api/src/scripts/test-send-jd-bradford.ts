/**
 * TEST: Send JD→Bradford Invoice PDFs to Nick
 *
 * This is a one-time test script to send all JD→Bradford invoices
 * to nick@jdgraphic.com for review.
 *
 * Run with: DATABASE_URL="file:./packages/db/prisma/dev.db" npx tsx apps/api/src/scripts/test-send-jd-bradford.ts
 */
...
async function main() {
  console.log('\n========================================');
  console.log('📄 TEST: SENDING JD→BRADFORD INVOICES TO NICK');
  console.log('========================================\n');
  ...
  if (pdfAttachments.length > 0) {
    ...
    try {
      await sendEmail({
        ...
        subject: `TEST - JD→Bradford Invoices (${pdfAttachments.length} files) - Net 10 Terms with Paper Usage & CPM`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background-color: #ffcc00; padding: 10px; border-radius: 5px; margin-bottom: 20px;">
              <strong>⚠️  TEST EMAIL - FOR REVIEW ONLY</strong>
            </div>

            <h2 style="color: #254979;">JD→Bradford Invoices</h2>

            <p>Hi Nick,</p>

            <p>This is a test email with all JD→Bradford invoices. The invoices include:</p>

            <ul style="line-height: 1.8;">
              <li><strong>Payment Terms:</strong> Net 10 Days</li>
              <li><strong>Paper Usage Details:</strong> Total paper weight and per-1,000 breakdown</li>
              <li><strong>Size & Paper Type:</strong> Job specifications included on each invoice</li>
              <li><strong>CPM Rate:</strong> Cost per thousand rate displayed</li>
              <li><strong>Calculation Transparency:</strong> Complete calculation formula shown</li>
            </ul>

            <div style="background-color: #f0f4f8; padding: 15px; border-left: 4px solid #1074B3; margin: 20px 0;">
              <strong>Summary:</strong><br>
              ✅ ${success} invoices generated successfully<br>
              ${failed > 0 ? `❌ ${failed} invoices failed<br>` : ''}
              📎 ${pdfAttachments.length - 1} PDF files attached<br>
              📊 1 Excel summary sheet attached
            </div>

            <p>All invoice PDFs are attached to this email, along with an Excel summary spreadsheet containing:</p>
            <ul style="line-height: 1.8;">
            ...
            <p style="color: #666; font-size: 12px; margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd;">
              This is a test email generated by the Printing Workflow system.
            </p>
          </div>
        `,
      ...
      console.log(`✅ Email sent successfully to nick@jdgraphic.com`);
