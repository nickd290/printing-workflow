/**
 * Regenerate Bradford→Impact Invoice PDFs with Net 10 Terms
 *
 * This script regenerates PDFs for all Bradford→Impact invoices to update
 * the payment terms from Net 30 to Net 10 days.
 *
 * Run with: DATABASE_URL="file:./packages/db/prisma/dev.db" npx tsx apps/api/src/scripts/regenerate-bradford-impact-pdfs.ts
 */
...
async function generateInvoicePDF(invoice: any, job: any, fromCompany: any, toCompany: any, po: any) {
  ...
  // Bradford→Impact invoices use Net 10 Days
  ...
  let refY = y + 10;
  if (po && po.poNumber) {
    page.drawText(`Impact PO: ${po.poNumber}`, { x: 60, y: refY, size: 9, font, color: accentColor });
    refY -= 13;
  }
  ...
  // Add size and paper type if available
  let jobDetailsY = y - 5;
  if (job.sizeName) {
    page.drawText(`Size: ${job.sizeName}`, { x: 320, y: jobDetailsY, size: 9, font, color: mediumGray });
    jobDetailsY -= 13;
  }
  if (job.paperType) {
    page.drawText(`Paper: ${job.paperType}`, { x: 320, y: jobDetailsY, size: 9, font, color: mediumGray });
  }
  ...
  if (job.paperWeightTotal) {
    ...
    page.drawText(paperText, { x: 60, y, size: 8, font, color: mediumGray });
    y -= 12;
  }

  // CPM Rate (if available)
  if (po && po.vendorCPM) {
    ...
    if (job.quantity) {
      const thousandUnits = job.quantity / 1000;
      const calculatedTotal = thousandUnits * cpmRate;
      const calcText = `Calculation: ${job.quantity.toLocaleString()} pcs \u00F7 1,000 \u00D7 $${cpmRate.toFixed(2)} = $${calculatedTotal.toFixed(2)}`;
...
async function main() {
  console.log('\n========================================');
  console.log('📄 REGENERATING BRADFORD→IMPACT INVOICE PDFs (NET 10)');
  console.log('========================================\n');

  // Find all Bradford→Impact invoices
  const bradfordImpactInvoices = await prisma.invoice.findMany({
    where: {
  ...
  console.log(`Found ${bradfordImpactInvoices.length} Bradford→Impact invoices\n`);
  ...
  for (const invoice of bradfordImpactInvoices) {
    try {
      console.log(`Processing ${invoice.invoiceNo} for job ${invoice.job.jobNo}...`);

      // Find the relevant PO for this invoice (Impact→Bradford PO)
      ...
      // Generate new PDF with Net 10 terms
      ...
      console.log(`  ✅ Regenerated PDF with Net 10 terms\n`);
      success++;
    } catch (error: any) {
  ...
  if (excelData.length > 0) {
    ...
    try {
      ...
      const worksheet = workbook.addWorksheet('Bradford→Impact Invoices');

      // Define columns
      worksheet.columns = [
        { header: 'Invoice #', key: 'invoiceNo', width: 18 },
        { header: 'Job #', key: 'jobNo', width: 18 },
        { header: 'Impact PO', key: 'impactPO', width: 18 },
      ...
      // Add to attachments
      pdfAttachments.push({
        filename: 'Bradford-Impact-Invoice-Summary.xlsx',
  ...
  if (pdfAttachments.length > 0) {
    ...
    try {
      await sendEmail({
        to: 'brandon@impactdirectprinting.com',
        cc: 'nick@starterboxstudios.com',
        subject: `Updated Bradford→Impact Invoices (${pdfAttachments.length} invoices) - Net 10 Terms with Paper Usage & CPM`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #254979;">Updated Bradford→Impact Invoices</h2>

            <p>Hello Brandon,</p>

            <p>All Bradford→Impact invoices have been regenerated with the following updates:</p>

            <ul style="line-height: 1.8;">
              <li><strong>Payment Terms:</strong> Net 10 Days (updated from Net 30)</li>
              <li><strong>Paper Usage Details:</strong> Now includes total paper weight and per-1,000 breakdown</li>
              <li><strong>CPM Rate:</strong> Displays the cost per thousand rate</li>
              <li><strong>Calculation Transparency:</strong> Shows the complete calculation formula</li>
            </ul>
            ...
            <p>All updated invoice PDFs are attached to this email, along with an Excel summary spreadsheet containing:</p>
            <ul style="line-height: 1.8;">
              <li>Invoice and job numbers</li>
              <li>Invoice and due dates</li>
              <li>Invoice amounts</li>
              <li>Paper usage (total lbs and per-1,000 breakdown)</li>
              <li>CPM rates and calculation formulas</li>
            </ul>
      ...
      console.log(`✅ Email sent successfully to brandon@impactdirectprinting.com`);
      console.log(`   CC: nick@starterboxstudios.com`);
      console.log(`   Attachments: ${pdfAttachments.length} PDF files\n`);
    } catch (error: any) {
