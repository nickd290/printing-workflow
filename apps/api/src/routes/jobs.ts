...
export const jobRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // NOTE: Pricing fields are automatically recalculated when quantity/sizeId/customPrice change
  // Direct updates to calculated pricing fields are rejected
  fastify.patch('/:id', async (request, reply) => {
    ...
    const body = request.body as {
      ...
      quantity?: number;
      sizeId?: string;
      customPrice?: number; // Override customer price (triggers recalculation)
      deliveryDate?: string;
    ...
    if (body.customPrice !== undefined) updates.customPrice = body.customPrice;
